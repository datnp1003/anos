    ...
    pub fn switch(&mut self, id: &str) -> Result<String> {
        if let Some(p) = self.providers.get(id) {
            ...
            let list: Vec<String> = self
                .providers
                .values()
                .map(|p| format!("{} — {}/{}", p.id(), p.name(), p.model()))
                .collect();
            anyhow::bail!(
                "Provider '{}' not found.\nAvailable:\n  {}",
                id,
