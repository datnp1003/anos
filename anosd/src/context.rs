...
impl PromptContext {
    pub fn load(dir: &str) -> Result<Self> {
        ...
        tracing::info!("System prompt ({} chars), {} skills", sp.len(), skills.len());
        Ok(Self { system_prompt: sp, skills })
    }

    fn load_system_prompt(dir: &str) -> Result<String> {
        let content = std::fs::read_to_string(&format!("{}/ANOS-SYSTEM-PROMPT.md", dir))?;
        let body = if content.starts_with("---") {
            content[4..].find("---").map(|e| content[4+e+4..].trim().to_string()).unwrap_or(content)
        } else { content };
    ...
    fn load_skills(dir: &str) -> Result<HashMap<String, Skill>> {
        ...
        for entry in std::fs::read_dir(path)? {
            ...
            let (name, desc, body) = if content.starts_with("---") {
                ...
                (extract_yaml(fm, "name"), extract_yaml(fm, "description"), body)
            } else {
                (entry.file_name().to_string_lossy().to_string(), String::new(), content)
            };
            skills.insert(name.clone(), Skill { name, description: desc, body });
        }
    ...
    pub fn find_skill(&self, hint: &str) -> Option<&Skill> {
        self.skills.iter().find(|(n, s)| {
            n.contains(hint) || s.description.contains(hint) || s.body.contains(hint)
        }).map(|(_, s)| s)
    }

    pub fn build_system_prompt(&self, skill_name: Option<&str>, system_map: Option<&str>, intent_hint: Option<&str>) -> String {
        let mut prompt = self.system_prompt.clone();
        if let Some(map) = system_map { prompt.push_str("\n\n---\n\n"); prompt.push_str(map); }
